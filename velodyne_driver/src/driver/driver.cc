                               ...
                               ros::NodeHandle private_nh)
{
  ...
  if (dump_file != "")                  // have PCAP file?
  ...
  else
    {
      // read data from live socket
        position_input_.reset(new velodyne_driver::InputSocket(private_nh, POSITION_PORT_NUMBER));//生成监听8308端口的对象
  ...
  std::thread thread(&VelodyneDriver::poll_positioning_packet, this);//生成新线程，用于监听位置包数据
  thread.detach();//新线程分离
}
...
void VelodyneDriver::poll_positioning_packet(void)//监听位置包函数，当小时标志位为false时进入循环调用getpositionpacket函数
{
    while(!hour)
    {
        int rc =position_input_->getPositionPacket();
        if(rc==0)break;
    }
}
