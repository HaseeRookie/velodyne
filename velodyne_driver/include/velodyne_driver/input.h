...
namespace velodyne_driver
{
  ...
  static bool hour= false;//获取小时数标志位，1为获取成功
  static uint64_t hourtonsecs=0;//整点小时数转换的时间戳
  static uint32_t pkttime=0;//离上个整点的微秒数，也用于判断小时数是否增加
  ...
  public:
    ...
    virtual int getPositionPacket() = 0;//获取数据包函数
  ...
  public:
    ...
    virtual int getPositionPacket();//获取数据包函数
  ...
  private:
    int sockfd_;
    in_addr devip_;
    bool input_available(int timeout);//判断端口是否连接成功
  };
  ...
  public:
    ...
    virtual int getPacket(velodyne_msgs::VelodynePacket *pkt, 
                          const double time_offset);
    virtual int getPositionPacket();//获取位置函数，无操作
    void setDeviceIP( const std::string& ip );
